...
export function NewsCard({ news }: NewsCardProps) {
  ...
  return (
    <div ref={cardRef} className="w-full h-full max-w-full md:max-w-xl mx-auto bg-white relative flex flex-col md:h-[95dvh] md:rounded-[2rem] md:shadow-2xl overflow-hidden animate-in fade-in duration-500">
      ...
      >
        {news.video_url ? (
          <div className="relative w-full h-full" onClick={toggleVideo}>
            ...
            <div className="absolute bottom-2 right-2 z-20 share-ignore">
              <div className="bg-black/40 backdrop-blur-md text-white p-1.5 rounded-md text-[10px] font-bold border border-white/10 uppercase tracking-tighter flex items-center gap-1">
        ...
        <div className="absolute bottom-4 left-4 z-20 select-none pointer-events-none drop-shadow-2xl opacity-100">
          {branding?.appLogo ? (
            ...
            <div className="flex items-center gap-2 text-white opacity-80">
              ...
              <span className="font-headline font-black text-base tracking-tighter uppercase">News Pulse</span>
            </div>
          )}
        </div>
        ...
        <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
          <div className="flex gap-2">
            <div className="bg-primary/90 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase flex items-center gap-1 shadow-lg backdrop-blur-sm tracking-widest">
              {categoryLabel}
            </div>
            <div className="bg-black/60 text-white px-3 py-1 rounded-full text-[10px] font-bold flex items-center gap-1 shadow-lg backdrop-blur-sm border border-white/10">
              <MapPin className="w-3 h-3" />
              {news.location.mandal}, {news.location.district}
            </div>
          </div>
          <div className="bg-black/40 text-white px-3 py-1 rounded-full text-[10px] font-mono flex items-center gap-1 shadow-lg w-fit backdrop-blur-sm border border-white/10 opacity-70">
      ...
      {/* Content Section */}
      <div className="px-5 py-4 flex-1 flex flex-col overflow-y-auto bg-gradient-to-b from-slate-50/50 to-white touch-pan-y">
        <div className="space-y-4 pb-12 md:pb-6">
          
          {/* Reporter Header */}
          <div className="flex items-center justify-between gap-3 mb-2 bg-white/80 p-2 rounded-xl border border-slate-100 shadow-sm backdrop-blur-sm">
            <div className="flex items-center gap-2.5 flex-1 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-sm font-black truncate text-slate-900 leading-none mb-1">{news.author_name || "Reporter"}</span>
            ...
            {/* Interactions */}
            <div className="flex items-center gap-4 shrink-0 pr-1 share-ignore">
              ...
              <Sheet>
                ...
                <SheetContent side="bottom" className="h-[80dvh] rounded-t-[2.5rem] p-0 z-[100] border-none shadow-2xl overflow-hidden">
                  <SheetHeader className="p-5 border-b bg-white/50 backdrop-blur-md sticky top-0 z-10">
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-4 pb-40">
                      {comments && comments.length > 0 ? (
                        comments.map((comment: any) => (
                          <div key={comment.id} className="bg-slate-50 p-4 rounded-xl border border-slate-100 animate-in slide-in-from-bottom-2">
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground/50">
                          <MessageCircle className="w-10 h-10 opacity-10 mb-3" />
                          <p className="font-bold text-sm">ఇంకా కామెంట్స్ ఏవీ లేవు.</p>
                        </div>
                      )}
                    </div>
                    <div className="fixed bottom-0 left-0 right-0 p-4 pb-12 bg-white border-t flex gap-3 items-center">
                      <Input 
                        placeholder="కామెంట్ జోడించండి..." 
                        value={newComment}
                        className="rounded-full h-12 bg-slate-100 border-none text-sm px-5"
                        onChange={(e) => setNewComment(e.target.value)}
                        onKeyDown={(e) => e.key === 'Enter' && handleAddComment()}
                      />
                      <Button size="icon" className="rounded-full h-12 w-12 shrink-0 shadow-lg shadow-primary/20" onClick={handleAddComment}>
              ...
              <button onClick={handleShare} disabled={isSharing} className="group flex flex-col items-center gap-1">
                {isSharing ? <Loader2 className="w-5 h-5 animate-spin text-primary" /> : <Share2 className="w-5 h-5 text-slate-400 group-hover:text-primary" />}
                <span className="text-[9px] font-black text-slate-700 uppercase">Share</span>
              </button>
            </div>
          </div>
          ...
          <button 
            ...
            className="flex flex-col items-center justify-center py-6 opacity-30 hover:opacity-100 transition-opacity md:hidden w-full group share-ignore"
          >
            <p className="text-[10px] font-black text-primary uppercase tracking-[0.2em] mb-2">మరిన్ని వార్తలు</p>
          ...
          <DialogContent className="max-w-[95vw] w-full h-[80vh] p-0 border-none bg-black/95 flex items-center justify-center rounded-[2rem] overflow-hidden z-[110] shadow-2xl">
            ...
            <div className="relative w-full h-full p-4 flex items-center justify-center">
              <div className="relative w-full h-full">
                ...
                <div className="absolute bottom-8 left-8 flex items-center gap-3 select-none pointer-events-none opacity-80">
