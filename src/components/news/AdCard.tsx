...
export function AdCard({ ad }: AdCardProps) {
  ...
  return (
    <div className="w-full h-full max-w-full md:max-w-xl mx-auto bg-black relative flex flex-col md:h-[95dvh] md:rounded-[2.5rem] md:shadow-2xl overflow-hidden animate-in fade-in duration-500">
      <div className="relative w-full h-full">
        ...
        <div className="absolute top-4 left-4 z-20">
          <div className="bg-black/40 backdrop-blur-md text-white px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 border border-white/10 shadow-2xl">
            <Megaphone className="w-3.5 h-3.5 text-primary animate-pulse" />
            Sponsored
          </div>
        </div>
        ...
        <div className="absolute bottom-12 left-0 right-0 p-8 flex flex-col items-center gap-6 text-center">
          <div className="space-y-2">
            ...
            <h3 className="text-white font-black text-2xl leading-tight">హైపర్ లోకల్ ఆఫర్స్ కోసం ఇక్కడ క్లిక్ చేయండి</h3>
          </div>

          {ad.link && (
            <Button 
              size="lg" 
              className="rounded-full h-16 px-10 text-lg font-black bg-primary text-white shadow-[0_0_40px_rgba(var(--primary),0.4)] hover:scale-105 transition-transform group"
