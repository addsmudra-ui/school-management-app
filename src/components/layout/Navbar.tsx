...
export function Navbar() {
  ...
  return (
    <nav className={cn(
      ...
      isMinimized ? "h-14 opacity-95 backdrop-blur-md translate-y-1 md:translate-y-0" : "h-16 opacity-100"
    )}>
      <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
        
        <Sheet open={isLegalOpen} onOpenChange={setIsLegalOpen}>
          <SheetTrigger asChild>
            <button className={cn("flex items-center gap-2 font-bold text-xl shrink-0", theme.text)}>
              <div className="relative w-8 h-8 flex items-center justify-center overflow-hidden shrink-0">
                {branding?.appLogo ? <Image src={branding.appLogo} alt="Logo" fill className="object-contain" priority /> : <Newspaper className="w-6 h-6" />}
              </div>
          ...
          <SheetContent side="left" className="w-[85%] sm:max-w-sm p-0 z-[110] border-none shadow-2xl">
            <SheetHeader className={cn("p-6 border-b", theme.bg)}>
              <div className="flex flex-col items-center gap-4">
                <div className="relative w-14 h-14 flex items-center justify-center overflow-hidden shrink-0">
                  {branding?.appLogo ? <Image src={branding.appLogo} alt="Logo" fill className="object-contain" priority /> : <Newspaper className="w-10 h-10 text-primary" />}
                </div>
                <SheetTitle className="text-xl font-black tracking-tight">{branding?.appName || 'Telugu News Pulse'}</SheetTitle>
              </div>
            </SheetHeader>
            
            <div className="flex flex-col h-full overflow-y-auto pb-32">
              <div className="p-5 space-y-5 border-b">
                <p className="text-[11px] font-black uppercase text-muted-foreground tracking-widest px-2 mb-2">Sections (న్యూస్ సెక్షన్స్)</p>
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => handleCategorySelect('All')} className={cn("flex items-center gap-3 p-3 rounded-xl transition-all border", selectedCategory === 'All' ? "bg-primary text-white border-primary shadow-md" : "bg-slate-50 border-transparent hover:bg-slate-100")}>
                    <Globe className={cn("w-5 h-5", selectedCategory === 'All' ? "text-white" : "text-primary")} />
                    <span className="text-sm font-bold">All (అన్నీ)</span>
                  </button>
                  {dynamicCategories.map((cat: any) => (
                    <button key={cat.value} onClick={() => handleCategorySelect(cat.value)} className={cn("flex items-center gap-3 p-3 rounded-xl transition-all border", selectedCategory === cat.value ? "bg-primary text-white border-primary shadow-md" : "bg-slate-50 border-transparent hover:bg-slate-100")}>
              ...
              {userName && (
                <div className="p-5 border-b bg-slate-50/50">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-full border bg-white overflow-hidden shrink-0">
                      {userPhoto ? <Image src={userPhoto} alt={userName} width={48} height={48} className="object-cover" /> : <User className="w-full h-full p-2.5 text-muted-foreground" />}
                    </div>
              ...
              <div className="p-5 space-y-5">
                <div className="space-y-2">
                  <p className="text-[11px] font-black uppercase text-muted-foreground tracking-widest px-2 mb-2">Legal & Information</p>
                  <Link href="/privacy" onClick={() => setIsLegalOpen(false)} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors group"><Shield className="w-5 h-5 text-primary group-hover:scale-110 transition-transform" /><span className="text-sm font-bold">Privacy Policy</span><ChevronRight className="w-4 h-4 ml-auto text-muted-foreground/50" /></Link>
                  <Link href="/guidelines" onClick={() => setIsLegalOpen(false)} className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors group"><FileText className="w-5 h-5 text-primary group-hover:scale-110 transition-transform" /><span className="text-sm font-bold">Terms & Conditions</span><ChevronRight className="w-4 h-4 ml-auto text-muted-foreground/50" /></Link>
                  <div className="p-4 rounded-xl bg-amber-50/50 border border-amber-100">
                    <div className="flex items-center gap-2.5 mb-2"><AlertTriangle className="w-4 h-4 text-amber-600" /><span className="text-[11px] font-black uppercase tracking-tight text-amber-800">Disclaimer</span></div>
                    <p className="text-xs text-amber-900 leading-relaxed font-medium opacity-80">ఈ యాప్‌లో ప్రచురించబడే వార్తలు మరియు అభిప్రాయాలు సంబంధిత రిపోర్టర్లవే. ప్లాట్‌ఫారమ్ ఏవైనా వాస్తవాలకు లేదా వార్తా కంటెంట్‌కు బాధ్యత వహించదు.</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="flex flex-1 justify-around md:justify-end md:gap-8 items-center h-full">
          <Link href="/" className={cn("flex flex-col md:flex-row items-center gap-1 text-muted-foreground transition-all duration-300", theme.hover, isMinimized ? "opacity-0 translate-y-4 pointer-events-none w-0" : "opacity-100")}><Home className="w-5 h-5" /><span className="text-[11px] md:text-sm font-semibold">Home</span></Link>

          <Dialog open={isLocationModalOpen} onOpenChange={setIsLocationModalOpen}>
            <DialogTrigger asChild><button className={cn("flex flex-col md:flex-row items-center gap-1 text-muted-foreground transition-all duration-300 relative px-4", theme.hover, isMinimized ? "scale-110" : "scale-100")}><MapPin className={cn("w-5 h-5", !isMinimized && theme.text)} />{!isMinimized && <span className="text-[11px] md:text-sm font-semibold max-w-[80px] truncate">{location.district === "All" ? "Global" : location.mandal === "All" ? location.district : location.mandal}</span>}</button></DialogTrigger>
            <DialogContent className="w-[94%] max-w-sm rounded-[2.5rem] p-10 border-none shadow-2xl">
              <DialogHeader><DialogTitle className="text-2xl font-black text-center mb-6">ప్రాంతాన్ని ఎంచుకోండి</DialogTitle></DialogHeader>
              <div className="space-y-8">
                <div className="space-y-3">
                  <label className="text-[11px] font-black text-primary uppercase tracking-widest ml-1">జిల్లా (District)</label>
                  <Select value={location.district} onValueChange={(val) => handleLocationUpdate(val, "All")}>
                    <SelectTrigger className="w-full h-14 rounded-2xl border-slate-100 bg-slate-50 font-bold text-sm"><SelectValue placeholder="జిల్లాను ఎంచుకోండి" /></SelectTrigger>
                    <SelectContent><SelectItem value="All">అన్ని జిల్లాలు (All Districts)</SelectItem>{Object.keys(dynamicLocations).sort().map((d) => <SelectItem key={d} value={d} className="font-bold text-sm">{d}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                {location.district !== "All" && (
                  <div className="space-y-3">
                    <label className="text-[11px] font-black text-primary uppercase tracking-widest ml-1">మండలం (Mandal)</label>
                    <Select value={location.mandal} onValueChange={(val) => handleLocationUpdate(location.district, val)}>
                      <SelectTrigger className="w-full h-14 rounded-2xl border-slate-100 bg-slate-50 font-bold text-sm"><SelectValue placeholder="మండలాన్ని ఎంచుకోండి" /></SelectTrigger>
                      <SelectContent><SelectItem value="All">అన్ని మండలాలు</SelectItem>{dynamicLocations[location.district]?.map((m: string) => <SelectItem key={m} value={m} className="font-bold text-sm">{m}</SelectItem>)}</SelectContent>
                    </Select>
                  </div>
                )}
                <Button className="w-full h-14 text-base font-bold rounded-2xl shadow-xl shadow-primary/20" onClick={() => setIsLocationModalOpen(false)}>వార్తలు చూడండి</Button>
              </div>
            </DialogContent>
          </Dialog>
          ...
          <Sheet open={isNotifOpen} onOpenChange={(open) => { setIsNotifOpen(open); if (open) markAsRead(); }}>
            ...
            <SheetContent side="right" className="w-[85%] sm:max-w-sm p-0 z-[100] border-none shadow-2xl">
              ...
              <div className="flex flex-col h-full overflow-y-auto p-4 space-y-4 pb-32">
                {notifications?.map((n) => (
                  <div key={n.id} onClick={() => handleNotifClick(n.postId)} className={cn("p-4 rounded-xl border transition-all group", n.postId ? "cursor-pointer bg-white border-muted hover:shadow-md" : "bg-muted/20 border-transparent")}>
                    <div className="flex justify-between items-start mb-1"><Badge variant="secondary" className={cn("text-[9px] font-bold uppercase px-2 border-none", theme.bg, theme.text)}>{n.target}</Badge><span className="text-[9px] text-muted-foreground">{n.timestamp?.toDate ? format(n.timestamp.toDate(), 'h:mm a') : 'Just now'}</span></div>
                    <h4 className="font-bold text-base mb-1 line-clamp-1">{n.title}</h4>
